...
class HttpClient {
  ...
  HttpClient({required this.baseUrl, this.defaultTimeout = const Duration(seconds: 10)});
  ...
  Future<http.Response> _send(
    ...
    return switch (request.method) {
      ...
      HttpMethod.post => _client
          .post(uri, headers: headers, body: jsonEncode(request.body))
          .timeout(timeout),
      HttpMethod.put => _client
          .put(uri, headers: headers, body: jsonEncode(request.body))
          .timeout(timeout),
      HttpMethod.delete => _client.delete(uri, headers: headers).timeout(timeout),
      HttpMethod.patch => _client
  ...
  Future<ApiResult> get(
    ...
    Map<String, String>? headers,
  }) => execute(HttpRequest.get(path, queryParameters: queryParameters, headers: headers));
