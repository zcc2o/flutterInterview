  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const Text('Dart 单线程模型：同步代码 → 清空微任务 → 取出一个事件 → 清空微任务 → ...', style: TextStyle(fontSize: 11, color: Colors.grey)),
      ],
    );
  }
}
...
class _LoopPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final center = Offset(120, 100);
    final r = 70.0;

    // 主循环圆
    canvas.drawCircle(center, r, Paint()..color = Colors.grey.shade200..style = PaintingStyle.stroke..strokeWidth = 3);

    // 标签
    _drawLabel(canvas, '同步代码\nexecute', Offset(center.dx, center.dy - 55), Colors.blue);
    _drawLabel(canvas, 'Microtask\nQueue', Offset(center.dx, center.dy + 45), Colors.teal);
    _drawLabel(canvas, 'Event Queue\n(Future/Timer)', Offset(center.dx + 140, center.dy - 10), Colors.orange);

    // 箭头
    final arrowPaint = Paint()..color = Colors.grey..strokeWidth = 1.5;
    final p1 = Offset(center.dx + r * cos(-pi / 2), center.dy + r * sin(-pi / 2));
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawRRect(rect, Paint()..color = Colors.orange.shade50..style = PaintingStyle.fill);
      canvas.drawRRect(rect, Paint()..color = Colors.orange.shade200..style = PaintingStyle.stroke);
    }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawRRect(rect, Paint()..color = Colors.teal.shade50..style = PaintingStyle.fill);
      canvas.drawRRect(rect, Paint()..color = Colors.teal.shade200..style = PaintingStyle.stroke);
    }
  }
...
class _ConceptTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rows = [
      ...
      ('Microtask', 'scheduleMicrotask,\nFuture.microtask, .then', '同步代码后、事件前执行，可插队'),
